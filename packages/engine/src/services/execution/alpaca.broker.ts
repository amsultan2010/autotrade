...
export class AlpacaBroker implements BrokerProvider {
  ...
  async submitOrder(order: BrokerOrder): Promise<BrokerOrderResult> {
    const crypto = isCryptoOrderSymbol(order.symbol);
    const side = order.side === 'LONG' ? 'buy' : 'sell';
    const priceHint = order.limitPrice ?? order.stopLoss ?? order.takeProfit ?? 1;
    const normalizedQty = normalizeAlpacaQty(order.qty, priceHint, crypto);
    ...
    if (order.type === 'limit' && order.limitPrice != null) {
      body.limit_price = roundAlpacaPrice(order.limitPrice, crypto);
    }

    // Bracket orders work for equities; crypto uses market entry + in-app stop/target monitoring.
    const canBracket =
      !crypto &&
      order.stopLoss != null &&
    ...
    if (canBracket) {
      ...
      body.stop_loss = { stop_price: roundAlpacaPrice(order.stopLoss!, false) };
      body.take_profit = { limit_price: roundAlpacaPrice(order.takeProfit!, false) };
    }

    try {
      return await this.postOrder(body, order);
    } catch (err) {
      if (!canBracket) throw err;
      // Bracket rejected (price bands, buying power, etc.) — still enter the position.
      const fallback: Record<string, unknown> = {
        ...
        type: order.type,
        time_in_force: crypto ? 'gtc' : 'day',
        client_order_id: `${order.clientOrderId}-mkt`,
      };
      return await this.postOrder(fallback, order);
    }
  }

  private async postOrder(body: Record<string, unknown>, order: BrokerOrder): Promise<BrokerOrderResult> {
