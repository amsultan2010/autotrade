...
function marketRiskOff(market: MarketContext | undefined): boolean {
  ...
  if (market.vix != null && market.vix >= 28) return true; // elevated fear
  if (market.spyTrend === 'DOWN' && market.qqqTrend === 'DOWN') return true;
  if (market.breadthPct != null && market.breadthPct < 30) return true;
  return false;
}
