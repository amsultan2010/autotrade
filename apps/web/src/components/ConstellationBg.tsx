...
const PARTICLE_COUNT_FULL = 90;
const PARTICLE_COUNT_GHOST = 52;
...
export function ConstellationBg({
  ...
  const resolved = mode ?? (dim ? 'dim' : 'full');
  const interactive = resolved === 'full';
  ...
  useEffect(() => {
    ...
    const count = resolved === 'ghost' ? PARTICLE_COUNT_GHOST : PARTICLE_COUNT_FULL;
    const particles = Array.from({ length: count }, () => makeParticle(w, h));
    const rain = resolved === 'full' ? Array.from({ length: RAIN_COUNT }, () => makeRainCol(w, h)) : [];
    let rafId: number;

    const particleAlpha =
      resolved === 'ghost' ? 0.5 : resolved === 'dim' ? 0.38 : 0.7;
    const connectionAlpha =
      resolved === 'ghost' ? 0.14 : resolved === 'dim' ? 0.1 : 0.22;
    const speedScale = resolved === 'ghost' ? 0.45 : resolved === 'dim' ? 0.65 : 1;
    ...
    function draw() {
      ...
      if (interactive) {
        ...
        if (mx > -100) {
          ...
          c.fillStyle = g;
          c.beginPath();
          c.arc(mx, my, 120, 0, Math.PI * 2);
          c.fill();
        }
      }

      // ── Drift particles (no mouse repel unless interactive) ──
  ...
  const canvasOpacity =
    resolved === 'ghost' ? 0.52 : resolved === 'dim' ? 0.4 : 0.55;
